"""QQ Mail read-only IMAP summary backend."""
...
def _opaque_reference(uid: bytes) -> str:
    digest = hashlib.sha256(b'qq_mail|' + uid).hexdigest()
...
def _parse_fetch_response(
    ...
    return BackendEmail(
        ...
        summary='仅从 QQ IMAP 邮件头元数据生成；未读取正文',
        summary_source='IMAP_HEADER_METADATA',
        read_state_changed=False,
        message_reference=_opaque_reference(uid),
        reference_kind='IMAP_UID_OPAQUE',
    )


@dataclass
class QqImapReadonlyBackend:
    config: QqImapConfig
    ...
    def summarize_today(self, max_emails: int) -> MailBackendResult:
        if not self.config.is_configured:
            return MailBackendResult(
                ...
                f'QQ IMAP 用户名未配置；请设置 {QQ_IMAP_USERNAME_ENVIRONMENT}',
            )
        try:
            ...
            return MailBackendResult(
                BackendStatus.IMAP_NOT_CONFIGURED,
                '无法从 Windows Credential Manager 读取 QQ IMAP 授权码',
            )
        if not authorization_code:
            return MailBackendResult(
                BackendStatus.IMAP_NOT_CONFIGURED,
                'QQ IMAP 授权码在 Windows Credential Manager 中不可用',
            )
        ...
        try:
            ...
            if status != 'OK':
                return MailBackendResult(
                    ...
                    'QQ IMAP 认证失败；请检查账号、授权码和 IMAP 开关',
                )
            authenticated = True
            ...
            for uid in reversed(uids[-max_emails:]):
                ...
                if status != 'OK' or not isinstance(fetch_data, list):
                    continue
                email = _parse_fetch_response(uid, fetch_data)
                if email is None:
                    continue
                parsed_count += 1
            ...
            if parsed_count == 0:
                return MailBackendResult(
                    ...
                    'QQ IMAP 找到今日候选邮件，但邮件头或日期无法解析',
                )
            if not emails:
                return MailBackendResult(
                    BackendStatus.EMPTY_TODAY,
                    'QQ IMAP 已解析候选邮件，确认今天没有邮件',
                )
            return MailBackendResult(
                BackendStatus.READY,
            ...
            return MailBackendResult(
                ...
                'QQ IMAP 协议命令失败' if authenticated
                else 'QQ IMAP 认证失败；请检查授权码和 IMAP 开关',
            )
        except (OSError, socket.timeout, ssl.SSLError):
            return MailBackendResult(
                BackendStatus.IMAP_NETWORK_FAILED,
                'QQ IMAP 网络或 TLS 连接失败',
            )
        except Exception as error:
            return MailBackendResult(
                BackendStatus.IMAP_PROTOCOL_ERROR,
                f'QQ IMAP 响应解析失败：{type(error).__name__}',
            )
        finally:
