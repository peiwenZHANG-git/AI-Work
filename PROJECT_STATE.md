...
- 当前晨报运行目录为 `D:\21781\Documents\Codex\AI-Work-v1.1-morning-brief`，任务直接引用该目录及已安装的Python310/pythonw。任务需要用户已登录和电脑可运行，不依赖ChatGPT、MCP、浏览器或助手页面。2026-09-06 已用生产通知路径真实运行：Python退出码0，Toast PowerShell成功，diagnostics记录`notification=ok`；三个artifact均更新，Downloads顶层元数据快照前后一致。
